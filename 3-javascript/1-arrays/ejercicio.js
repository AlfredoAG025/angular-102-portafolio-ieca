...
function vaciarCarritoConTotal() {
    ...
    // Mientras que el tamano de carrito sea mayor que 0
    while(carrito.length){
        // Elimina el primer elemento, lo recupera y acumula el precio al total
...
function cuentaDeAlumnos() {
    ...
    const alumnosGruposAB = grupoA.concat(grupoB);

    console.log(alumnosGruposAB.length);
}
...
function transmitiendoBatch() {
    ...
    const tamanoBloque = backup.length / 4;
    for(let i = 0; i < backup.length; i += tamanoBloque){
        const bloque = backup.slice(i, tamanoBloque * (i + 1));
        console.log(bloque, tamanoBloque);
    }
}
